...
$(function(){
    ...
    $('#list_data').datagrid({
        ...
        columns:[[      //每个列具体内容
            ...
            var allRows = $('#list_data').datagrid('getRows');
            var orderNo = 0;
            if (allRows.length > 0) {
                orderNo = $('#list_data').datagrid('getData').rows[allRows.length - 1].orderNo;
            }
        /*    orderNo = maxBuyNo >= buyNo ? maxBuyNo : buyNo;*/
           /* $('#list_data').datagrid('appendRow', {
                ...
                firmId: row.firmId,
                amount: row.amount,
                performNurse: row.performNurse,
                dosage: row.dosage,
                dosageUnits: row.dosageUnits,
                administration: row.administration,
                frequency: row.frequency,
                abidance: row.abidance,
                charges: row.charges,
                itemClass: row.itemClass,
                freqDetail: row.freqDetail,
                providedIndicator: row.providedIndicator,
                skinFlag:row.skinFlag
            });*/
        },
        frozenColumns:[[
            {field:'ck',checkbox:true}
