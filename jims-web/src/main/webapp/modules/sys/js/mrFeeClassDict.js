function onloadMethod() {
    $('#list_data').datagrid({
        ...
        fit: true,//自动大小
        url:basePath+'/MrFeeClassDict/list',
        //sortName: 'code',
        //sortOrder: 'desc',
        remoteSort: false,
        idField: 'fldId',
        ...
        columns: [[      //每个列具体内容
            ...
            {field: 'mrFeeClassCode', title: '费用分类代码', width: '10%', align: 'center'},
            {field: 'mrFeeClassName', title: '费用分类名称', width: '10%', align: 'center'},
            {field: 'inputCode', title: '输入码', width: '18%', align: 'center'},
            {field: 'mrFeeClassDesc', title: '费用分类描述', width: '18%', align: 'center'},
...
function del(id) {
    $.ajax({
        'type': 'POST',
        'url': basePath + '/MrFeeClassDict/del',
 ...
 */
function saveDict() {
    $.postForm(basePath + '/MrFeeClassDict/save', 'mrFeeClassDictForm', function (data) {
        if (data.data == 'success') {
...
function get(id) {
    ...
    $.ajax({
        'type': 'post',
        'url': basePath + '/MrFeeClassDict/get',
        'contentType': 'application/json',
        'data': id = id,
        'dataType': 'json',
        'success': function (data) {
            $('#mrFeeClassDictForm').form('load', data);
        }
    });
}
